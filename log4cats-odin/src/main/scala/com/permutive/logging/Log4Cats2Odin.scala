...
object Log4Cats2Odin {
  abstract class Log4CatsOdinLogger[F[_]: Applicative](logger: SelfAwareStructuredLogger[F], level: Level = Level.Info)
      extends OdinLogger[F] {

    implicit private def renderMessage[M](msg: => M)(implicit render: Render[M]): String = render.render(msg)
    implicit private def toThrowable[E](e: E)(implicit toThrowable: ToThrowable[E]): Throwable =
      toThrowable.throwable(e)

    override def minLevel: Level = level

    override def withMinimalLevel(level: Level): OdinLogger[F] = new Log4CatsOdinLogger[F](logger, level) {}
  ...
  def convert[F[_]: Applicative](logger: SelfAwareStructuredLogger[F], level: Level = Level.Info): OdinLogger[F] =
